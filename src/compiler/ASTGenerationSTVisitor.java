...
public class ASTGenerationSTVisitor extends FOOLBaseVisitor<Node> {
	...
	public Node visitTimesDiv(TimesDivContext c) {
		if (print) printVarAndProdName(c);
		if(c.TIMES() != null){
			final Node n = new TimesNode(visit(c.exp(0)), visit(c.exp(1)));
			n.setLine(c.TIMES().getSymbol().getLine());
			return n;
		}else{
			final Node n = new DivNode(visit(c.exp(0)), visit(c.exp(1)));
	...
	public Node visitPlusMinus(PlusMinusContext c) {
		if (print) printVarAndProdName(c);
		if(c.PLUS() != null){
			final Node n = new PlusNode(visit(c.exp(0)), visit(c.exp(1)));
			n.setLine(c.PLUS().getSymbol().getLine());
			return n;
		}else{
			final Node n = new MinusNode(visit(c.exp(0)), visit(c.exp(1)));
	...
	public Node visitAndOr(AndOrContext c){
		if (print) printVarAndProdName(c);
		if(c.AND() != null){
			final Node n = new AndNode(visit(c.exp(0)), visit(c.exp(1)));
			n.setLine(c.AND().getSymbol().getLine());
			return n;
		}else{
			final Node n = new OrNode(visit(c.exp(0)), visit(c.exp(1)));
	...
	public Node visitComp(CompContext c) {
		if (print) printVarAndProdName(c);
		if(c.EQ() != null){
			final Node n = new EqualNode(visit(c.exp(0)), visit(c.exp(1)));
			n.setLine(c.EQ().getSymbol().getLine());
			return n;
		} else if (c.LE() != null) {
			final Node n = new LessEqualNode(visit(c.exp(0)), visit(c.exp(1)));
			n.setLine(c.LE().getSymbol().getLine());
			return n;
		}else{
			final Node n = new GreaterEqualNode(visit(c.exp(0)), visit(c.exp(1)));
	...
	public Node visitCldec(CldecContext c) {
		...
		for (int i = 1; i < c.ID().size(); i++) {
			final FieldNode field = new FieldNode(c.ID(i).getText(),(TypeNode) visit(c.type(i-1)));
			field.setLine(c.ID(i).getSymbol().getLine());
			fields.add(field);
		}

		final List<MethodNode> methodsList = new ArrayList<>();
		for(int i = 0; i < c.methdec().size(); i++){
			final MethodNode method = (MethodNode) visit(c.methdec(i));
			methodsList.add(method);
		}

		final Node n = new ClassNode(c.ID().get(0).getText(), fields, methodsList);
	...
	public Node visitMethdec(MethdecContext c) {
		...
		for (int i = 1; i < c.ID().size(); i++) {
			final ParNode par = new ParNode(c.ID(i).getText(),(TypeNode) visit(c.type(i)));
		...
		for (DecContext dec : c.dec()){
			declarations.add((DecNode) visit(dec));
		}

		Node n = null;
		if (c.ID().size()>0){
			n = new MethodNode(c.ID(0).getText(), (TypeNode)visit(c.type(0)), parameters, declarations, visit(c.exp()));
			n.setLine(c.FUN().getSymbol().getLine());
		}
		return n;
	}
	...
	public Node visitNew(NewContext c) {
		...
		for (int i = 0; i < c.exp().size(); i++) {
			final Node field = visit(c.exp(i));
			fields.add(field);
		}

		final Node n = new NewNode(c.ID().getText(), fields);
	...
	public Node visitDotCall(DotCallContext c) {
		...
		final List<Node> parList = new ArrayList<>();
		for (int i = 0; i < c.exp().size(); i++) {
			final Node f = visit(c.exp(i));
			parList.add(f);
		}

		final Node n = new ClassCallNode(c.ID(0).getText(), c.ID(1).getText(), parList);
		n.setLine(c.ID(0).getSymbol().getLine());
		return n;
	}
}
